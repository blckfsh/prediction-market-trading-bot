  ...
  try {
    const before = await client.query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE "marketVariant" = 'SPORTS_MATCH' AND ("tradeType" IS NULL OR BTRIM("tradeType") = ''))::int AS sports_missing,
        COUNT(*) FILTER (WHERE "marketVariant" <> 'SPORTS_MATCH' AND ("tradeType" IS NULL OR BTRIM("tradeType") = ''))::int AS non_sports_missing
    ...
    const sportsDefault = await client.query(`
      UPDATE "BuyPositionConfig"
      SET "tradeType" = 'na'
      WHERE "marketVariant" = 'SPORTS_MATCH'
    ...
    console.log('Filled SPORTS_MATCH missing -> na:', sportsDefault.rowCount);
    console.log(
      'Filled non-SPORTS_MATCH missing -> avg-price:',
