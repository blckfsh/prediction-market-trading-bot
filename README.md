...
- Shared rule settings are normalized through `BetRuleConfig` (`amount`, `profitTakingPercentage`, `status`, `priority`) to avoid duplicated schema.
- Sports head-to-head conflicts resolve deterministically via `BetRuleConfig.priority`.
...
- Uses `BetRuleConfig.priority` when multiple supported teams match the same sports slug.
